...
- **Обновлено 2026-08-11 — outline pill, лёгкое начертание:** заливка убрана, border `0.5px solid` акцентного цвета на 50% прозрачности (тоньше и приглушённее, чем чистый `text-info`), текст `text-info` на `weight-regular` (было `weight-medium`) — «тонкие, невесомые» по прямому запросу Daria. `radius-pill` (было `radius-lg`, но не полная пилюля). Все 4 варианта визуально идентичны — статус читается по копирайту, не по цвету. Цвет прошёл 2 итерации: `accent-900` (~1.1–1.2:1 на тёмных поверхностях — нечитаемо) → `text-info`/`info-500` (~2.6–3.0:1, подтверждено Daria).
- **Обновлено 2026-08-17 — обводка переведена на токен:** border был прибит как `rgba(75,52,245,0.5)` напрямую в коде компонента, в обход Semantic Variables — из-за этого пережил ребрендинг на Petrol и остался на старом индиго, пока не был замечен на реальном экране. Заменён на `border-info-subtle` (новый токен, `ds/foundation.md` → Обводки), сейчас резолвится в `rgba(28,126,160,0.5)`. См. `ds/CONTRACT.md` → «Нет смыслового токена» — это ровно тот случай, ради которого правило существует.
...
- **Обновлено 2026-08-17 (тот же день, позже) — расстояние между марком и wordmark уменьшено:** `.brand { gap: var(--space-1) }` = **4px** (было `var(--space-3)` = 12px сразу после добавления марка). Марк и wordmark должны читаться как единый плотный лого-блок, а не два раздельных элемента с воздухом между ними. Тот же `--space-1`/4px зафиксирован в `landing/landing.css` → `.nav__brand` — марк и wordmark в App TopNav и в Landing nav используют идентичный gap. Любой производный лого-блок (питчдеки, соцсети, презентации) обязан наследовать это же значение 4px, а не собственную интерпретацию отступа.
...
- **Структура (обновлено 2026-08-11 — паттерн Onboarding infoBanner):** одиночный div, composes pageBlock напрямую — выравнивается по левому/правому краю наравне с остальным контентом страницы (max-width 1440, centered), НЕ full-bleed. Shadow и иконка убраны, но фон и обводка вернулись — Banner теперь визуально повторяет `.infoBanner`/`.successBanner` со step 1/3 Onboarding: тонкая `border-focus`-обводка + `bg-{variant}-subtle` заливка.
...
- **Status:** минималистичный пасс (фон/бордер/тень/иконка убраны) применён 2026-08-11 ко всем использованиям — компонент общий, правка каскадная
