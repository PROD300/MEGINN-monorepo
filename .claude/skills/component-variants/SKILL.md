...
description: Дополнить существующий компонент в дизайн-системе недостающими variants, состояниями и/или размерами — расширить Component Set до матрицы variants × states × sizes с привязками через Semantic Variables, обновив ds/components.md и ds/foundation.md (новые Semantic-токены под состояния, если потребовались). Работает в режиме single (для одного компонента) или bulk (проход по всему ds/components.md). Используй, когда нужно нарастить матрицу вариантов/состояний у уже существующего в ДС компонента.
...
- Готовишь UI-кит к этапу сборки экранов
...
Если строка отсутствует — переспроси: «компонента `<Name>` нет в `ds/components.md`. Это новый компонент? Тогда нужен `grow_ui_kit` (/grow-ui-kit), не `component_variants`. Запустить?»
...
- Все добавления — alias-привязки, не прибитые значения. Это инвариант контракта `ds/CONTRACT.md`
...
```markdown
> Расширено: добавлены состояния hover/disabled/loading и размеры sm/md/lg.
