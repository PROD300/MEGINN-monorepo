---
name: prd-to-sitemap
description: Превращает продуктовый PRD в sitemap и user flow — сначала в текстовых файлах проекта с согласованием на каждом шаге, затем (только после финального апрува пользователя) отрисовывает их на FigJam через Figma MCP (/figma-mcp). Используй, когда PRD готов и нужно построить информационную архитектуру (дерево экранов с приоритетами) и user flow конкретного сценария, прежде чем переходить к wireframes.
disable-model-invocation: true
---

# PRD → Sitemap + User Flow
...
Только после апрува всех текстовых артефактов. См. справочник по MCP-инструментам Figma и правила работы с ними в `/figma-mcp`.
