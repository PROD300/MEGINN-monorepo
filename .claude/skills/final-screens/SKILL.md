---
name: final-screens
description: Собрать финальный экран продукта в Figma, заменив каркасные прямоугольники из wireframe на instance реальных компонентов из UI-кита, с привязками к ДС и проработанными состояниями (включая edge cases — empty / loading / overflow). На входе — wireframe-фрейм и каталог ds/components.md. На выходе — фрейм Screen/<Name> в SectionNode «Screens», composition map в ds/screens/<name>.md, и записи used-in у задействованных компонентов. Работает в режиме single (один экран) или bulk (проход по всем wireframes). Используй, когда нужно перейти от wireframe к продакшн-готовому экрану с реальными компонентами ДС.
...
- Создали в UI-ките через short-cycle `grow_ui_kit` (/grow-ui-kit) (Phase 2.5) **в том же прогоне `final_screens`**
...
Если попутно понадобились новые семантические токены (например, `surface-activity-income` / `surface-activity-outcome`) — добавить в `ds/foundation.md` Slой 2 как alias к существующим Primitive, синхронизировать в Variables через тот же short-cycle (расширение `ds_baseline` / /ds-baseline). Это инвариант: **новые токены добавляются как alias, не как прибитые значения**.
...
Если Effects в foundation нет — пред-этап `ds_baseline` (/ds-baseline) должен был их положить. Если их там реально нет (legacy-проект) — сообщи пользователю и предложи добавить.
...
- Подписи / placeholder-тексты, которые проставил агент ранее

Если фрейма нет — переспроси: «wireframe `<ScreenName>` не найден. Создавать его сейчас? Это задача другой директивы (/wireframes), не текущей. Запустить `wireframes`?»
...
- Если у компонента есть строка `used in:` — её тоже сохрани, понадобится для аудита (/screens-audit)
...
1. Сначала добавить компонент в ДС через grow_ui_kit (/grow-ui-kit) (рекомендую)
...
На «да, выносим» — добавляется в очередь Phase 2.5 (inline grow_ui_kit). На «оставить локально» — собирается как domain-composition в финальном экране, помечается в md-карте «при появлении 2-го экрана с этим блоком — вынести через grow_ui_kit (/grow-ui-kit)».
...
Если в Phase 1.5 пользователь подтвердил вынос N компонентов — эта фаза происходит **до** Phase 4 (canvas-write финального экрана). Логика — провернуть mini-цикл `grow_ui_kit` (/grow-ui-kit) внутри текущего прогона, чтобы потом в Phase 4 ставить `createInstance()` от только что созданных мастеров, а не frame-копии.

### Фаза 2.5.1: Бриф каждого нового компонента

Для каждого подтверждённого компонента — короткий бриф (см. формат в `/grow-ui-kit` Phase 2):
...
Один или несколько вызовов `use_figma` (по правилу из `/grow-ui-kit`: ≤3 компонента на вызов). В каждом:
...
5. Все привязки fills / strokes / cornerRadius / itemSpacing / fontSize — через Variables и TextStyles, как в `/grow-ui-kit` Phase 3 правилах 1-7

После создания — короткий audit (как `/grow-ui-kit` Phase 3.3): нет ли unbound fills, нет ли FRAME-вместо-COMPONENT, все ли тексты с `textStyleId`.
...
1. **Все компоненты в composition map существуют в `ds/components.md`?** Если есть призрак — стоп, либо в Phase 1.3 (берём из ДС), либо в Phase 1.5 / 2.5 (выносим новый компонент сейчас), либо в директиву `grow_ui_kit` (/grow-ui-kit) отдельным прогоном.
