# OBSIDIAN — фичи под капотом: что реализовано, что заявлено в PRD
...
- Account abstraction, self-custody: OBSIDIAN не имеет кастодии над активами пользователя.
...
- Статусы операций (pending/success/failed) отслеживаются; кто отвечает за сбой стороннего моста — прописано в Legal Terms (не OBSIDIAN).
...
- Задокументированный замысел (`prd/features_list.md` F11, `briefs/obsidian/personas.md`): 2-3-уровневая цепочка одобрения (например, CFO → CEO → Treasury) с автоматическим роутингом по сумме/типу операции.
