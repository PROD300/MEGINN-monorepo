...
2. **`landing/`** — маркетинговый сайт (сырой HTML/CSS/JS-набросок), **не хостится у пользователя** (Daria), дорабатывается **только в Figma**, не в коде.
...
3. **`landing/`** — дорабатывается только в Figma, в коде не трогать, никуда не деплоить (не хостится у пользователя).
