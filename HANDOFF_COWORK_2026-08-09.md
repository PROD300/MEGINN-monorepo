...
>
> **Обновление 2026-08-11:** схема хэндовера изменена — второй агент больше **не коммитит и не пушит сразу**. Новый порядок: развернуть на dev-сервере → дать Daria на аппрув → коммитить и пушить только после её «да» (так же, как уже устроено по `app/`). Отменяет предыдущее правило «коммитить и пушить после каждой команды». Подробности — в блоке «ОБНОВЛЕНИЕ ПРОЦЕССА» в начале `HANDOFF_COWORK_2026-08-10.md`, это правило действует для всех последующих хэндоверов, не только текущего.
